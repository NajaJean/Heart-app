...
@RestController
@RequestMapping("/api")
public class OldRealTimeController {

	@Autowired
	OldRealTimeRepository oldRealtimeRepository;

	// @PostMapping("/realtime")
	// public ResponseEntity<RealTime> createRealTime(@RequestBody RealTime
	// realtime) {
	// try {
	...
	// if (r.getDatepost().equals(null) || r.getMeasurementvalue().length == 0 ||
	// r.getPatientid().equals("")) {
	...
	// getLatestPatientsECG(@PathVariable("patient_id") String patient_id) {
	// try {
	// List<RealTime> ecgs =
	// realtimeRepository.findFirstByPatientidOrderByDatepostDesc(patient_id);
